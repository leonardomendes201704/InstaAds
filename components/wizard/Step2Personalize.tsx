...
import type { AdStyle, PublishTarget } from "@/lib/types";
...
const publishOptions: Array<{
  ...
  { id: "feed", label: "Feed", icon: <Grid3X3 className="h-5 w-5" /> },
  { id: "stories", label: "Stories", icon: <Circle className="h-5 w-5" /> },
  { id: "both", label: "Ambos", icon: <Layers className="h-5 w-5" /> },
];
...
export function Step2Personalize() {
  ...
  return (
    <>
      ...
      subtitle="Escolha formato, estilo e mensagem para a IA criar sua arte."
      onBack={prevStep}
      onClose={() => {
      ...
      footer={
        <GradientButton
          ...
          disabled={!mainMessage.trim() || isGenerating}
          onClick={() => generateAd()}
        >
          Continuar →
        </GradientButton>
    ...
    >
      <div className="flex h-full flex-col gap-3 overflow-hidden">
        ...
        <section>
          <p className="mb-2 text-sm font-medium">Onde publicar?</p>
          <div className="grid grid-cols-3 gap-2">
            {publishOptions.map((option) => (
              <OptionCard
                ...
                label={option.label}
                icon={option.icon}
                selected={publishTarget === option.id}
        ...
        <section className="min-h-0 flex-1">
          <p className="mb-2 text-sm font-medium">Mensagem principal</p>
          <input
            type="text"
            value={mainMessage}
